import sys,time,datetime,copy,subprocess,itertools,pickle,warnings,numbers,multiprocessing,logging,signal
...
try:
    from .timeout import raise_timeout
except:
    from timeout import raise_timeout
...
## MCMC
##################################
def MCMC(model,n_sample,n_core=1,prior=[],opt=[]):

    logging.info( "  -- MCMC n_sample:%d n_core:%d" % (n_sample,n_core) )

    #determine step size
    logging.info('  -- MCMC determine step size')
    step_size_list = np.array([0.06,0.08,0.1,0.12,0.15,0.2,0.25,0.3,0.4,0.5])
    m = 500
    args_list = [ [model,m,step_size,np.random.randint(2**32-1),prior,opt,False] for step_size in step_size_list ]
    [_,_,dtl1] = MCMC_prl(args_list,n_core)
    opt_step_size = dtl1.iloc[ np.argmin(np.fabs(dtl1['r_accept'].values-0.5)) ]['step_size']

    #mcmc
    logging.info('  -- MCMC start sampling')
    logging.info( "  -- estimated processing time: %.2f minutes" % (dtl1['elapsed_time'].mean()*n_sample/m/60.0) )
    args_list = [ [model,n_sample,opt_step_size,np.random.randint(2**32-1),prior,opt,True] for _ in range(n_core) ]
    [para_mcmc,L_mcmc,dtl2] = MCMC_prl(args_list,n_core)
    dtl_mcmc = {'step_size':opt_step_size,'dtl1':dtl1,'dtl2':dtl2}

    return [para_mcmc,L_mcmc,dtl_mcmc]

def MCMC_prl(args_list,n_core):

    sigterm = signal.getsignal(signal.SIGTERM)

    p = multiprocessing.Pool(n_core)

    def kill(signalnum,handler):
        logging.info('  -- MCMC_prl SIGTERM received')
        try:
            p.terminate()
            p.join()
            logging.info('  -- MCMC_prl killed child process')
        except:
            logging.info('  -- MCMC_prl no child process')
        raise_timeout()

    signal.signal(signal.SIGTERM,kill)

    rslt = [ p.apply_async(MCMC_core,args=args) for args in args_list ]
    p.close()
    p.join()
    rslt = [ rslt_i.get() for rslt_i in rslt ]

    signal.signal(signal.SIGTERM,sigterm)

    para_mcmc     = pd.concat([ rslt_i[0] for rslt_i in rslt ],ignore_index=True)
    L_mcmc        = np.hstack([ rslt_i[1] for rslt_i in rslt ])
    step_size     = np.array([  rslt_i[2] for rslt_i in rslt ])
    r_accept      = np.array([  rslt_i[3] for rslt_i in rslt ])
    elapsed_time  = np.array([  rslt_i[4] for rslt_i in rslt ])
    dtl = pd.DataFrame(np.vstack([step_size,r_accept,elapsed_time]).T,columns=['step_size','r_accept','elapsed_time'])

    return [para_mcmc,L_mcmc,dtl]

def MCMC_core(model,n_sample,step_size,seed=0,prior=[],opt=[],adjust_step_size=True):
    ...
    para1 = param.from_dict(model.para)
    m = para1.shape[0]
    m_exp = param.idx('para_exp').shape[0]
    m_ord = param.idx('para_ord').shape[0]

    ##step
    step_MCMC = param.from_dict(model.ste)
    step_MCMC[param.idx('para_exp')] = np.minimum( np.log( 1.0 + step_MCMC[param.idx('para_exp')]/para1[param.idx('para_exp')] ) ,0.4)
    step_MCMC *= step_size
    ...
    while 1:
        para2 = np.zeros_like(para1)
        para2[param.idx('para_ord')] = para1[param.idx('para_ord')] + np.random.randn(m_ord)*step_MCMC[param.idx('para_ord')]
        para2[param.idx('para_exp')] = para1[param.idx('para_exp')] * np.exp( np.random.randn(m_exp)*step_MCMC[param.idx('para_exp')] )
    ...
    para_mcmc = pd.DataFrame(np.array(para_mcmc),columns=model.stg['para_list'])
...
def Quasi_Newton(model,prior=[],merge=[],opt=[]):
    ...
    if 'ste' in opt:
        ste = param.to_dict(EstimationError(model,para,prior))
    else:
        ste = []
