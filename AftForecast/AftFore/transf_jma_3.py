...
#Long, Lat, Year, Month, Day, Magnitude, Depth, Hour, Minute (with decimals)
...
dtype1 = np.dtype([('year', 'i4'), ('month', 'i4'), ('day', 'i4'), ('hour', 'i4'), ('minute', 'f8')]) 
...
#ref_hour = 19
#ref_min = 14.29
# Reference time (for Gorj sequence, ROMPLUS catalog)
ref_year = 2023
ref_month = 2
ref_day = 14
ref_hour = 13
...
# Find the number of rounded minutes, seconds and miliseconds
# Find seconds (integer) and microseconds
curr_min_int = np.array([])
curr_sec_int = np.array([])
curr_microsec = np.array([])
for i in range(len(curr_min)):
    curr_min_intl = math.floor(curr_min[i])
    curr_secl = (curr_min[i] - curr_min_intl)*60
    curr_sec_intl = math.floor(curr_secl)
    curr_microsecl = int((curr_secl - curr_sec_intl)*1000000)
    curr_min_int = np.append(curr_min_int, curr_min_intl)
    curr_sec_int = np.append(curr_sec_int, curr_sec_intl)
    curr_microsec = np.append(curr_microsec, curr_microsecl)

ref_min_int = math.floor(ref_min)
ref_sec = (ref_min - ref_min_int)*60
ref_sec_int = math.floor(ref_sec)
ref_microsec = int((ref_sec - ref_sec_int)*1000000)

# Find the time difference
ref_time = dt.datetime(ref_year,ref_month,ref_day,ref_hour,int(ref_min_int),int(ref_sec_int),int(ref_microsec))
time_dif_list=[]
for i in range(len(curr_min)):
#    print i, int(curr_min_int[i])
    time_vec = dt.datetime(curr_year[i],curr_month[i],curr_day[i],curr_hour[i],int(curr_min_int[i]),int(curr_sec_int[i]),int(curr_microsec[i]))
    #Changed to days
