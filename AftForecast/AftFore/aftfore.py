...
def calc_c_cum_exp(para,mag_ref,t):
    ...
    st = t['st']; en = t['en'];
#   Changed the sampling of magnitude: influences fore.txt and fore.pdf (Bogdan Enescu)
#   mag = np.arange(0.95,10.0,0.1).reshape(-1,1); mag0 = mag[0,0];
    mag = np.arange(0.45,10.0,0.1).reshape(-1,1); mag0 = mag[0,0];
...
def Graph_dr(Data,mu,t,para,xlim=None,ylim=None,newfig=True,pdf=False):
    ...
    #Added the following line to mark Mc = mu + 2*signa (Bogdan Enescu)
    plt.plot(T,mu+2*sigma,'r--',linewidth=1)
    #It (the magnitude threshold for the forecasting period) can be calculated also here, but I calculated it in the calling script (Bogdan Enescu)
    #Mcomplete = mu[-1]+2*sigma #magnitude of completeness (95%)
    #print( "Mc: %f" % Mcomplete )
    ...
    plt.xscale('log')
    plt.xlabel('Time after the mainshock (days)')
...
#   Modified limits
    ylim = [0.8,1000.0] if ylim is None else ylim
    xlim = [1.0,6.0] if xlim is None else xlim
    #ylim = [0.8,100.0] if ylim is None else ylim
    #xlim = [0.5,5.0] if xlim is None else xlim
    mag = pred['mag']
    ...
    #Modified title (Bogdan Enescu)
    #plt.title('K: %.2e, p: %.2f, c: %.2e, beta: %.2f, t_:[%.3f,%.3f], ML: %.1f'%(k,p,c,beta,t1,t2,m0))
    plt.title('K: %.2e, p: %.2f, c: %.2e, beta: %.2f'%(k,p,c,beta))
...
def pred_dist(param,t_test,Data_test=None):
    ...
    np.savetxt('fore.txt',np.hstack([mag.values.reshape(-1,1),pred[['expected_num','lower_bound','upper_bound','prob']].values]),fmt='%.2f %15.3f %11d %11d %11.4f',header='(ML) (expected_number) (lower bound of 95% interval) (upper bound of 95% interval) (probability)')
