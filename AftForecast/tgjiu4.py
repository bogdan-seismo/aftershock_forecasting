...
if __name__ == '__main__':
    ...
    #Data (for Gorj and so on)
    Data    = './AftFore/gorj.dat'    # The path of the date file
    #Data    = './AftFore/gorj_romplus.dat'    # The path of the date file
    
    #Periods
    t_learn = [0.0, 0.375]            # The range of the learning period [day]
    t_for   = [0.375, 0.750]          # The range of the testing period [day]

    #File with estimated parameters
    file_param = 'gorj_forecast_0h-9h_9h-18h.dat'
    ...
    mu_last = mu[-1]                               #Magnitude of detection (mu) at the end of the training window
    Mcomplete = mu[-1] + 2*param['para']['sigma']  #Magnitude of completeness (95%)
    mag_th = math.ceil(Mcomplete*10)/10            #The magnitude threshold mag_th for forecasting (round up Mcomplete)
